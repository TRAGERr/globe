...
switch _mode do
{
	case "eh_draw3d":
	{
		...
		//get positions for window icons
		private _windowPositions = GET_OBJVAR(_building,"#windowPositions",nil);
		if (isNil{_windowPositions}) then
		{
			_windowPositions = ["getBuildingWindows",[_module,_building]] call bis_fnc_moduleEditTerrainObject;
			SET_OBJVAR(_building,"#windowPositions",_windowPositions);
		};
		if (count _windowPositions == 0) exitWith {};
		private _windowFlags = _building getVariable ["#windowFlags", WINDOW_FLAGS_EMPTY];
		...
		//draw 3d icons on door and window positions
		...
		private ["_iconW","_stateW","_colorW"];
		{
			_stateW = _windowFlags select _forEachIndex;

			_iconW = [ICON3D_WINDOW_CLOSED,ICON3D_WINDOW_LOCKED,ICON3D_WINDOW_OPENED] select _stateW;
			_colorW = [COLOR_WINDOW_CLOSED,COLOR_WINDOW_LOCKED,COLOR_WINDOW_OPENED] select _stateW;

			drawIcon3D ["", _colorW, _x, 0.6, -0.85, 0, str (_forEachIndex + 1), 2, 0.045, "RobotoCondensedBold","right",false];
			drawIcon3D [_iconW, _colorW, _x, 0.8, 0.8, 0, "", 2];
		}
	...
	case "init":
	{
		...
		{
			_ehBuildingChanged = addMissionEventHandler 
			[
				"BuildingChanged",
				{
					...
					//setup all windows
					private _windowFlags = _parent getVariable ["#windowFlags", WINDOW_FLAGS_EMPTY];
					{
						SETUP_WINDOW(_current,_forEachIndex,_x);
					}
					forEach _windowFlags;
				}
			];
		...
		if (!is3DEN) then
		{
			...
			//set window states
			private _windowFlags = ["decodewindowFlags",[_module]] call bis_fnc_moduleEditTerrainObject;
			_building setVariable ["#windowFlags", _windowFlags];
			{
				SETUP_WINDOW(_building,_forEachIndex,_x);
			}
			forEach _windowFlags;
	...
	case "attributesChanged3DEN":
	{
		...
		if (!isNull _buildingPrev && {_building != _buildingPrev}) then
		{
			...
			//heal all windows
			_buildingPrev setVariable ["#windowFlags", nil];
			for "_windowID" from WINDOW_INDEX_MIN to WINDOW_INDEX_MAX do
			{
				SETUP_WINDOW_PREVIEW(_buildingPrev,_windowID-1,WINDOW_STATE_CLOSED);
			};
		};
		...
		//get and store building window positions
		private _windowPositions = GET_OBJVAR(_building,"#windowPositions",[]);
		...
		//get window states and flags
		private _windowFlags = ["decodeWindowFlags",[_module]] call bis_fnc_moduleEditTerrainObject;
		_building setVariable ["#windowFlags", _windowFlags];

		//parent: set window state and animate
		{
			SETUP_WINDOW_PREVIEW(_building,_forEachIndex,_x);
		}
		forEach _windowFlags;
		...
		if (isNull objNull && {_child != _building}) then
		{
			...
			_child setVariable ["#windowFlags", _windowFlags];
			{
				SETUP_WINDOW_PREVIEW(_child,_forEachIndex,_x);
			}
			forEach _windowFlags;
		};
	...
	case "unregisteredFromWorld3DEN":
	{
		...
		//heal all windows
		_building setVariable ["#windowFlags", nil];
		for "_windowID" from WINDOW_INDEX_MIN to WINDOW_INDEX_MAX do
		{
			SETUP_WINDOW_PREVIEW(_building,_windowID-1,WINDOW_STATE_CLOSED);
		};
	...
	case "getBuildingWindows":
	{
		private _building = _input param [1,objNull,[objNull]];
		if (isNull _building) exitWith {[]};

		private _cfg = configfile >> "CfgVehicles" >> typeOf _building >> "HitPoints";
		...
		for "_windowID" from WINDOW_INDEX_MIN to WINDOW_INDEX_MIN do
		{
			_position = getText(_cfg >> format["glass_%1_hitpoint",_windowID] >> "DestructionEffects" >> format ["BrokenGlass%1",_windowID] >> "position");
	...
	case "encodeWindowFlags":
	{
		private _flags = _input param [1,WINDOW_FLAGS_EMPTY,[[]]];
	...
	case "decodeWindowFlags":
	{
		private _value = _input param [1,_module getVariable ["#windowStates",[0,0,0]],[123,[]]];
		private _flags = [];

		//retype from old tech using single numeric value (with indexes from 0-15), each index can have one ot the 2 values (opened/closed)
		if (_value isEqualType 123) then
		{
			_flags = ([_value,WINDOW_INDEX_MAX] call bis_fnc_decodeFlags2) apply {if (_x == 0) then {WINDOW_STATE_CLOSED} else {WINDOW_STATE_OPENED}};
		}
		//handle new tech with 3 values with indexes 0-7 (3 rows & 8 columns), each index can have one ot the 3 values (opened/locked/closed)
		else
		{
			{_flags append ([_x,WINDOW_COLUMNS] call bis_fnc_decodeFlags4);} forEach _value;
		};
	...
	case "objectWindows_onMouseButtonUp":
	{
		(_input select 1) params ["_ctrlAttribute","_mouseButton","","","_shiftKey","_ctrlKey","_altKey"];

		private _state = _ctrlAttribute getVariable ["#state",WINDOW_STATE_CLOSED];

		if (_shiftKey || _ctrlKey || _altKey) then
		{
			_state = switch (true) do
			{
				case _altKey: {WINDOW_STATE_OPENED};
				case _shiftKey: {WINDOW_STATE_LOCKED};
				case _ctrlKey: {WINDOW_STATE_CLOSED};
			};
		}
		else
		{
			if (_mouseButton == 0) then
			{
				_state = [WINDOW_STATE_CLOSED,WINDOW_STATE_LOCKED,WINDOW_STATE_OPENED] select ((_state + 1) % 3);
			}
			else
			{
				_state = WINDOW_STATE_CLOSED;
			};
		};

		_ctrlAttribute setVariable ["#state",_state];
		_ctrlAttribute ctrlSetText ([TEXTURE_WINDOW_CLOSED,TEXTURE_WINDOW_LOCKED,TEXTURE_WINDOW_OPENED] select _state);
		_ctrlAttribute ctrlSetTooltip ([TEXT_WINDOW_CLOSED,TEXT_WINDOW_LOCKED,TEXT_WINDOW_OPENED] select _state);
	};
	...
	case "objectWindows_attributeLoad":
	{
		private _ctrlAttribute = _input param [1,controlNull,[controlNull]];
		private _value = _input param [2,0,[123,[]]];

		private _flags = ["decodeWindowFlags",[_module,_value]] call bis_fnc_moduleEditTerrainObject;
...

		{
			...
			{
				_ctrlCheckbox setVariable ["#state",_x];
				_ctrlCheckbox ctrlSetText ([TEXTURE_WINDOW_CLOSED,TEXTURE_WINDOW_LOCKED,TEXTUREWINDOW_OPENED] select _x);
			};

			_ctrlCheckbox ctrlSetTooltip ([TEXT_WINDOW_CLOSED,TEXT_WINDOW_LOCKED,TEXT_WINDOW_OPENED] select _x);
		}
		forEach _flags;
		...
		{
			getNumber (configfile >> "CfgVehicles" >> typeOf _building >> "numberOfWindows")
		};

		_available = _available min WINDOW_INDEX_MAX;

		for "_windowID" from 1 to WINDOW_INDEX_MAX do
		{
			_ctrlCheckbox = _ctrlCheckboxes controlsGroupCtrl (100+_windowID);
			if (_windowID <= _available) then {_ctrlCheckbox ctrlSetFade 0} else {_ctrlCheckbox ctrlSetFade 0.85};
			_ctrlCheckbox ctrlCommit 0;
		};
	};
	...
	case "objectWindows_attributeSave":
	{
		private _ctrlAttribute = _input param [1,controlNull,[controlNull]];
		private _ctrlCheckboxes = _ctrlAttribute controlsGroupCtrl 100;
		...
		for "_idc" from 101 to (100+WINDOW_INDEX_MAX) do
		{
			_flags pushBack ((_ctrlCheckboxes controlsGroupCtrl _idc) getVariable ["#stateW",WINDOW_STATE_CLOSED]);
		};

		private _value = ["encodeWindowFlags",[_module,_flags]] call bis_fnc_moduleEditTerrainObject;
