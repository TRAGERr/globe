...
_null = _this spawn {
	...
	_fn_formation = {
		...
		if (isNil {_agent getVariable "leader"}) then {		//--- for the first time in formation
			...
			if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): formation - agent set as a leader",_agent]};
			...
			if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): formation - leader found",_agent]};
			};
			...
			if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): formation - I'm leader, path finding",_agent]};
			...
			if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): formation - I'm not leader, path finding",_agent]};
	...
	_fn_safe = {
		//--- Move or wait
		if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): safe reaction start",_agent]};
	...
	_fn_danger = {
		if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): danger - start",_agent]};
		if (_state == "formation") then {call _fn_formationQuit};		//remove agent from members list and set leader to nil
		_state = "danger";
		_agent setVariable ["state",_state];
		_agent setVariable ["leader",nil];
	...
	_fn_reset = {
		if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): reset",_agent]};
	...
	if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): situation check - state:%2",_agent,_state]};
	...
	if !(isNil {BIS_fnc_animalBehaviour_debug}) then {debugLog format ["ANIMAL (%1): _stateDanger %2 -- _stateFormation %3 -- time from last check: %4",_agent,_stateDanger,_stateFormation,(time - _dbgTime)];_dbgTime = time};
			...
			case (time > _delaySound && _state != "danger") :
			{
				switch true do
				{
					...
					case (_agent isKindOf "Dog_Base_F"):
					{
						...
						{
							private _sound = "SoundSetSource_Sfx_Dog" createVehicle (getPosATL _agent);
							_sound attachTo [_agent, [0,0,0]];
						};
					};
