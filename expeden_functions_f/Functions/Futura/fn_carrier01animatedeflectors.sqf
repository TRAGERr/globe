...

{
	if (_forEachIndex == 0) then
	{
		switch (_animationState) do
		{
			case 10:
			{
				if (_carrierPart animationPhase _x < 0.1) then
				{
					[_carrierPart, ["Land_Carrier_01_blast_deflector_up_sound", 150, 1, 0, 0, false]] remoteExecCall ["say3D"];
				};
			};
			case 0:
			{
				if (_carrierPart animationPhase _x > 9.9) then
				{
					[_carrierPart, ["Land_Carrier_01_blast_deflector_down_sound", 150, 1, 0, 0, false]] remoteExecCall ["say3D"];
				};
			};
			default
			{
