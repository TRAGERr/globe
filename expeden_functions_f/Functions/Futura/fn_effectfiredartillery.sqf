	...
	[_pos1,_devPos] spawn {
		...
		[_var1_1,_var1_2,_var1_3,_pos1] spawn {
			...
			while {_i<_no} do {
				...
				[
					(_pos1 select 0) + (_var1_1/10) - 1.5 + random 2,
					(_pos1 select 1) + (_var1_2/10) - 1.5 + random 2,
					(_pos1 select 2) + (_var1_3/10) - 1.5 + random 2
				],
		...
		[_var1_1,_var1_2,_var1_3,_pos1] spawn {
			...
			while {_i<_no} do {
				...
				"", "Billboard", 1, 4+random 1,
				[
					(_pos1 select 0) + (-_var1_1/2 + _var1r),
					(_pos1 select 1) + (-_var1_2/2 + _var2r),
					(_pos1 select 2) + (-_var1_3/2 + _var3r)
				],
		...
		[_var2_1,_var2_2,_var2_3,_pos2] spawn {
			...
			while {_i<_no} do {
				...
				"", "Billboard", 1, 4+random 1,
				[
					(_pos2 select 0) + (_var2_2 - _var2r*2)/*  - 0.3 + random 0.6 */,
					(_pos2 select 1) + (-_var2_1 + _var1r*2)/*  - 0.3 + random 0.6 */,
					(_pos2 select 2)/*  - 0.3 + random 0.6 */
				],
		...
		[_var2_1,_var2_2,_var2_3,_pos3] spawn {
			...
			while {_i<_no} do {
				...
				"", "Billboard", 1, 4+random 1,
				[
					(_pos3 select 0) + (-_var2_2 + _var2r*2)/*  - 0.3 + random 0.6 */,
					(_pos3 select 1) + (_var2_1 - _var1r*2)/*  - 0.3 + random 0.6 */,
					(_pos3 select 2)/*  - 0.3 + random 0.6 */
				],
				[1*_var2r - 0.25 + (random 0.5),-1*_var1r - 0.25 + (random 0.5),- 0.25 + (random 0.5)],
				-16 + random 32, 1.276, 1, 0.8, 
				[0.2 + (1 * _r) + (_r2 * 1),1 + (5 * _r) + (_r2 * 3),2 + (8 * _r) + (_r2 * 5)],
